...
public class Exemple4Controller {
    ...
    public String searchMessages(@RequestParam("q") String query,
                                 @SessionAttribute("courant") String courant,
                                 Model model) {
        List<Message> results = facade.searchMessages(query);
        model.addAttribute("username", courant);
        model.addAttribute("received", facade.getReceivedMessage(courant));
        model.addAttribute("sent", facade.getSentMessage(courant));
        model.addAttribute("AAR", facade.searchMessages("AAR"));
        model.addAttribute("searchResults", results);
        model.addAttribute("users", facade.getAllUserNames());
        model.addAttribute("msgDto", new MessageDto());
