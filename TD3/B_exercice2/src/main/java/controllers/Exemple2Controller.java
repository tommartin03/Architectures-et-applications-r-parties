...
public class Exemple2Controller {
    ...
    @PostMapping("delete")
    public String delete(UserDto userDto,BindingResult result, Model model, @SessionAttribute("courant") String courant){
        try {
            facade.deleteUser(courant);
        } catch (Exception e) {
            result.addError(new ObjectError("user","Erreur lors de la suppression."));
            return "welcome";
        }
